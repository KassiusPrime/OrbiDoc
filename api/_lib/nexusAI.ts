...
export class CircuitBreaker {
  ...
  success() { this.failures = 0; this.state = 'CLOSED'; this.openedAt = 0; }
  failure() { this.failures += 1; if (this.failures >= (this.options.failureThreshold ?? 3)) { this.state = 'OPEN'; this.openedAt = Date.now(); } }
