...
const assertions = [
  ...
  [modules.includes("name: 'Documento'") && modules.includes("name: 'Planilha'") && modules.includes("name: 'Apresentação'"), 'Registry oficial não contém labels humanos dos módulos P1.'],
  [modules.includes("name: 'Design'") && modules.includes("name: 'PDF & OCR'") && modules.includes("name: 'Arquivos'"), 'Registry oficial não contém labels humanos de criação/arquivos.'],
  [modules.includes("name: 'Dashboards'") && modules.includes("name: 'Formulários'") && modules.includes("name: 'Agenda'"), 'Registry oficial não contém labels humanos P2/P3.'],
  ...
  [commandPalette.includes('Documento') && commandPalette.includes('Planilha') && commandPalette.includes('Apresentação'), 'Command Palette não usa labels humanos de criação.'],
  ...
  [fabMenu.includes('Documento') && fabMenu.includes('Planilha') && fabMenu.includes('Apresentação') && fabMenu.includes('Design') && fabMenu.includes('PDF & OCR'), 'Create Sheet não usa nomenclatura humana.'],
  ...
  [nexus.includes('Nexus AI') && nexus.includes('Gratuito'), 'Nexus AI não expõe identidade e estado gratuito discretos.'],
  [!nexus.includes('IA unificada · gratuita') && !nexus.includes('orquestração gratuita ativa'), 'Nexus AI voltou a expor badge de marketing de IA.'],
  [!nexus.includes('NexusMode') && !nexus.includes('modeLabels'), 'Nexus AI ainda possui tabs de modo.'],
  [nexus.includes('Como posso ajudar?') && nexus.includes('Escreva, analise arquivos ou peça ações no Orbit.'), 'Estado vazio do Nexus AI não segue o workspace quieto.'],
  [!nexus.includes('quickActions') && !nexus.includes('MediaToolRail'), 'Nexus AI ainda possui ações rápidas em cards/trilho.'],
  [!nexus.includes('Ferramentas de mídia') && !nexus.includes('mediaTools'), 'Nexus AI ainda possui rail de mídia.'],
  [nexus.includes('placeholder="Pergunte alguma coisa"') && nexus.includes('Adicionar contexto') && nexus.includes('Anexar arquivo') && nexus.includes('Web'), 'Composer quieto do Nexus AI não está completo.'],
  [nexus.includes('suggestionPrompts') && nexus.includes('Resumir') && nexus.includes('Analisar') && nexus.includes('Pesquisar') && nexus.includes('Criar'), 'Nexus AI não expõe até quatro chips compactos.'],
  [nexus.includes('Orbit está offline') && nexus.includes('rascunho foi preservado'), 'Nexus AI não falha honestamente quando offline.'],
  [nexus.includes('onSendToWord') && nexus.includes('Documento'), 'Nexus AI não oferece continuidade contextual para documentos.'],
  ...
  [home.includes('Documento') && home.includes('Planilha') && home.includes('Apresentação') && home.includes('PDF & OCR') && !home.includes('Orbit Nova') && !home.includes('Orbit Gravity'), 'Orbispace Home não usa labels humanos nem remove a nomenclatura cósmica.'],
...
console.log('Orbit UI audit OK: quiet Nexus workspace, human module labels, single visible speed dial, V2 shell, design tokens/primitives, command palette, safe areas, keyboard/IME, bottom navigation, create sheet and offline honesty are wired.');