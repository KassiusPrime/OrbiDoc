...
function read(file) {
  ...
  if (!fs.existsSync(full)) { failures.push(`${file} ausente.`); return ''; }
  return fs.readFileSync(full, 'utf8');
}
let config = null;
try { config = JSON.parse(read('capacitor.config.json')); } catch { failures.push('capacitor.config.json inválido.'); }
if (config) {
...
for (const token of ['isOrbiDocNativeRuntime', 'orbitApiUrl', 'https://doc-swiss.vercel.app']) if (!apiOrigin.includes(token)) failures.push(`orbitApiOrigin.ts não contém ${token}.`);
...
for (const token of ["orbitApiUrl('/api/chat')", "orbitApiUrl('/api/chat/stream')"]) if (!chatClient.includes(token)) failures.push(`src/api/chat.ts não usa ${token}.`);
...
for (const token of ['https://localhost', 'capacitor://localhost', 'Access-Control-Allow-Origin', 'OPTIONS']) if (!nativeCors.includes(token)) failures.push(`nativeCors.ts não contém ${token}.`);
for (const routeFile of ['api/chat.ts', 'api/chat/stream.ts']) if (!read(routeFile).includes('applyNativeCors')) failures.push(`${routeFile} não habilita preflight/CORS para o Capacitor.`);
const aiInternet = read('src/lib/aiInternet.ts');
for (const token of ['shouldUseAiInternet', 'ORBIT_NEXUS_WEB', 'webSearch: true']) if (!aiInternet.includes(token)) failures.push(`aiInternet.ts não contém ${token}.`);
...
if (!read('vite.config.ts').includes('src/lib/nativeFileSaver.ts')) failures.push('Vite não redireciona file-saver para o adaptador nativo.');
...
for (const token of ['@tesseract.js-data', "['por', 'eng']", 'tesseract.js-core']) if (!nativeAssetScript.includes(token)) failures.push(`prepare-native-offline-assets.mjs não contém ${token}.`);
const bridgeScript = read('scripts/configure-native-android-bridge.mjs');
for (const token of ['MediaStore.Downloads', 'OrbiDocNativePlugin', 'android.intent.action.VIEW', 'android.intent.action.SEND']) if (!bridgeScript.includes(token)) failures.push(`configure-native-android-bridge.mjs não contém ${token}.`);
const networkScript = read('scripts/configure-native-android-network.mjs');
for (const token of ['fetchUrlPayload', 'HttpURLConnection', '300L * 1024L * 1024L', 'android.permission.INTERNET']) if (!networkScript.includes(token)) failures.push(`configure-native-android-network.mjs não contém ${token}.`);
