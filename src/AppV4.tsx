...
export default function AppV4() {
  ...
  useEffect(() => {
    ...
    const saved = (activeProject.content as any)?.ocrItems;
    setOcrItems(Array.isArray(saved) ? saved : []);
  }, [activeProject?.id, view]);

  useEffect(() => {
    if (view !== 'extract' || activeProject?.type !== 'extract') return;
    const serializable = ocrItems.map((item) => ({
      ...item,
      fileUrl: item.fileUrl?.startsWith('blob:') ? undefined : item.fileUrl,
    }));
  ...
  const renderProjectMissing = (type: SavedProject['type']) => (
    <div className="max-w-xl mx-auto mt-14 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 text-center shadow-sm">
      ...
      <button onClick={() => createProject(type)} className="mt-5 h-10 px-4 rounded-xl bg-indigo-600 text-white text-xs font-black">Criar agora</button>
    </div>
  );
  ...
  const renderContent = () => {
    if (view === 'home') return (
      <HomeDashboard
        onNavigate={launchTool}
        onNewChat={() => navigate('chat')}
        recentHistory={history.slice(0, 6)}
        recentProjects={recentProjects}
        recentChats={[] as ChatSession[]}
        googleUser={googleUser}
        microsoftUser={microsoftUser}
        activeEngineLabel={selectedModel.label}
      />
    );
    ...
    if (view === 'projects') return (
      <FilesWorkspace
        ...
        onCreateProject={(type) => createProject(type)}
    ...
    if (view === 'history') return <HistoryVault
      ...
      onRestoreItem={(item) => {
        if (item.type === 'ocr') {
          ...
          setProjects((current) => [project, ...current]);
          setActiveProject(project);
          setOcrItems((project.content as any).ocrItems);
          setView('extract');
    ...
    <div className="h-dvh min-h-[560px] bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden flex">
      <aside className="hidden lg:flex w-[252px] shrink-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-col">
        <div className="h-16 px-4 flex items-center border-b border-slate-100 dark:border-slate-800">
          <button onClick={() => navigate('home')} aria-label="Ir para o início"><DocPlusLogo size="md" /></button>
        </div>
        <div className="p-3 flex-1 overflow-y-auto">
          <nav className="space-y-1">
            {NAV_ITEMS.map((item) => {
              ...
              return <button key={item.id} onClick={() => navigate(item.id)} className={`w-full h-10 px-3 rounded-xl flex items-center gap-3 text-xs font-bold ${active ? 'bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><Icon className="w-4 h-4" />{item.label}</button>;
            })}
          </nav>
          ...
          <div className="mt-2 space-y-1">
            {CREATE_ITEMS.map((item) => {
              const Icon = item.icon;
              return <button key={item.id} onClick={() => launchTool(item.id as TabType)} className="w-full h-9 px-3 rounded-xl flex items-center gap-3 text-[11px] font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/60"><Icon className={`w-4 h-4 ${item.iconClass}`} />{item.label}</button>;
            })}
          </div>
        </div>
        <div className="p-3 border-t border-slate-100 dark:border-slate-800">
          <button onClick={() => setInstallGuideOpen(true)} className="w-full rounded-xl border border-slate-200 dark:border-slate-700 p-3 text-left hover:bg-slate-50 dark:hover:bg-slate-800"><div className="text-xs font-black">Instalar DocSwiss</div><div className="text-[10px] text-slate-500 mt-0.5">PWA · WebAPK · TWA</div></button>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-16 shrink-0 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 sm:px-4 flex items-center gap-3 z-30">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-slate-400 hidden sm:block">{PROJECT_VIEWS.has(view) && activeProject ? 'Salvo localmente neste dispositivo' : online ? 'Online' : 'Modo offline'}</div>
          </div>

          <div className="relative ml-auto hidden md:block w-[250px] xl:w-[350px]">
            <Search className="w-4 h-4 absolute left-3 top-2.5 text-slate-400" />
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Criar ou buscar aplicativo…" className="w-full h-9 pl-9 pr-3 rounded-xl bg-slate-100 dark:bg-slate-950 border border-transparent focus:border-indigo-300 dark:focus:border-indigo-800 text-xs outline-none" />
        ...
        <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 h-[68px] border-t border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl px-2 pb-[env(safe-area-inset-bottom)] flex items-center justify-around">
          {[NAV_ITEMS[0], NAV_ITEMS[1], NAV_ITEMS[2], NAV_ITEMS[3], NAV_ITEMS[4]].map((item) => {
            const Icon = item.icon;
            const active = view === item.id || (item.id === 'chat' && (view === 'ai' || view === 'compare'));
            const label = item.label === 'Apps & converter' ? 'Apps' : item.label === 'Meus arquivos' ? 'Arquivos' : item.label;
            return <button key={item.id} onClick={() => navigate(item.id)} className={`min-w-[54px] h-12 rounded-xl flex flex-col items-center justify-center gap-1 text-[9px] font-bold ${active ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40' : 'text-slate-500 dark:text-slate-400'}`}><Icon className="w-5 h-5" />{label}</button>;
          })}
        </nav>
      </div>

      {menuOpen && <div className="lg:hidden fixed inset-0 z-50 bg-slate-950/50" onClick={() => setMenuOpen(false)}>
        <aside className="w-[300px] max-w-[88vw] h-full bg-white dark:bg-slate-900 shadow-2xl p-3 overflow-y-auto" onClick={(event) => event.stopPropagation()}>
          <div className="h-14 flex items-center justify-between px-2"><DocPlusLogo size="md" /><button onClick={() => setMenuOpen(false)} className="w-9 h-9 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center"><X className="w-5 h-5" /></button></div>
          <nav className="mt-3 space-y-1">{NAV_ITEMS.map((item) => { const Icon = item.icon; return <button key={item.id} onClick={() => navigate(item.id)} className="w-full h-11 px-3 rounded-xl flex items-center gap-3 text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800"><Icon className="w-4 h-4" />{item.label}</button>; })}</nav>
          <div className="mt-5 border-t border-slate-100 dark:border-slate-800 pt-4 space-y-1">{CREATE_ITEMS.map((item) => { const Icon = item.icon; return <button key={item.id} onClick={() => launchTool(item.id as TabType)} className="w-full h-10 px-3 rounded-xl flex items-center gap-3 text-[11px] font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"><Icon className={`w-4 h-4 ${item.iconClass}`} />Novo {item.label.toLowerCase()}</button>; })}</div>
          <button onClick={() => { setInstallGuideOpen(true); setMenuOpen(false); }} className="mt-5 w-full h-11 rounded-xl bg-indigo-600 text-white text-xs font-black">Instalar / Android</button>
        </aside>
      </div>}
