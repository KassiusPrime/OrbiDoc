...
export const AiWorkspace: React.FC<AiWorkspaceProps> = (props) => {
  ...
  const researchModel = useMemo(() => catalog.find((model) => model.provider === 'groq' && /compound/i.test(model.id)) || catalog.find((model) => /pesquisa|research|compound/i.test(`${model.label} ${model.id}`)) || null, [catalog]);
  ...
  return <div className="orbidoc-ai-studio space-y-3">
    <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 sm:px-4 py-3 shadow-sm flex flex-col xl:flex-row xl:items-center gap-3">
      ...
      <div className="orbidoc-product-tabs flex items-center bg-slate-100 dark:bg-slate-950 rounded-xl p-1 overflow-x-auto xl:ml-4"><button onClick={() => setArea('copilot')} className={`h-9 px-3 rounded-lg text-[11px] font-black inline-flex items-center gap-2 whitespace-nowrap ${area === 'copilot' ? 'bg-white dark:bg-slate-800 shadow-sm text-[#3157F6] dark:text-[#7AA2FF]' : 'text-slate-500'}`}><MessageSquare className="w-4 h-4" /> Copiloto</button><button onClick={() => setArea('research')} className={`h-9 px-3 rounded-lg text-[11px] font-black inline-flex items-center gap-2 whitespace-nowrap ${area === 'research' ? 'bg-white dark:bg-slate-800 shadow-sm text-[#3157F6] dark:text-[#7AA2FF]' : 'text-slate-500'}`}><WorldSearch className="w-4 h-4" /> Pesquisa Web</button></div>
    ...
    {area === 'copilot' ? <AiWorkspaceLegacy {...props} /> : <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 min-h-[calc(100dvh-11rem)]">
      <aside className="xl:col-span-4 space-y-4"><section className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm"><div className="inline-flex items-center gap-2 text-xs font-black text-[#3157F6] dark:text-[#7AA2FF]"><WorldSearch className="w-4 h-4" /> Pesquisa fundamentada</div><h2 className="mt-2 text-xl font-black">Pesquisar com fontes reais</h2><p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">Este modo usa um modelo com ferramenta de pesquisa, não apenas um prompt pedindo para “procurar”.</p><div className="mt-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3"><div className="text-[9px] font-black uppercase text-slate-400">Motor atual</div><div className="mt-1 text-xs font-black">{researchModel?.label || 'Nenhum motor de pesquisa ativo'}</div><div className="text-[9px] text-slate-400 mt-1">{researchModel ? `${providerLabel(researchModel.provider)} · ${researchModel.id}` : 'Ative Groq Compound ou equivalente.'}</div></div><div className="mt-4 space-y-2">{['Pesquise as notícias mais recentes sobre este assunto e cite fontes.', 'Compare as melhores opções atuais para esta decisão com fontes.', 'Faça uma pesquisa profunda: contexto, dados recentes, divergências e fontes.', 'Verifique esta afirmação na Web e indique o que é fato e o que é incerto.'].map((prompt) => <button key={prompt} onClick={() => setQuery(prompt)} className="w-full p-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-left text-[10px] font-bold hover:border-[#7AA2FF] hover:bg-[#EFF4FF]/50 dark:hover:bg-[#0D1E5B]/30">{prompt}</button>)}</div></section></aside>
      <section className="xl:col-span-8 rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden flex flex-col min-h-[620px]"><div className="p-4 border-b border-slate-100 dark:border-slate-800"><div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-2 focus-within:border-[#3157F6] focus-within:ring-2 focus-within:ring-[#3157F6]/10"><textarea value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && (event.ctrlKey || event.metaKey)) { event.preventDefault(); void runResearch(); } }} rows={4} className="w-full resize-y bg-transparent p-2 text-sm outline-none" placeholder="O que você quer pesquisar na Web?" /><div className="flex items-center gap-2"><span className="text-[9px] text-slate-400 hidden sm:inline">Ctrl/Cmd + Enter pesquisa</span><button disabled={busy || !query.trim() || !researchModel} onClick={() => void runResearch()} className="ml-auto h-10 px-4 rounded-xl bg-[#3157F6] hover:bg-[#2446D8] disabled:opacity-40 text-white text-xs font-black inline-flex items-center gap-2"><Search className="w-4 h-4" />{busy ? 'Pesquisando…' : 'Pesquisar'}</button></div></div></div><div className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="max-w-4xl mx-auto">{result ? <><div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5"><CleanMarkdown content={result} /></div><div className="mt-3 flex gap-2"><button onClick={() => navigator.clipboard.writeText(result)} className="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-black inline-flex items-center gap-2"><Copy className="w-4 h-4" /> Copiar</button>{onSendToWord && <button onClick={() => onSendToWord(result)} className="h-9 px-3 rounded-xl bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 text-[10px] font-black inline-flex items-center gap-2"><FileText className="w-4 h-4" /> Abrir em Documentos</button>}</div></> : <div className="py-24 text-center text-slate-400"><WorldSearch className="w-16 h-16 mx-auto opacity-25" /><div className="mt-4 text-sm font-black">Resultado da pesquisa aparecerá aqui</div><div className="mt-1 text-xs">Peça fontes, datas, comparação e verificação quando precisar.</div></div>}</div></div></section>
    </div>}
  </div>;
};
