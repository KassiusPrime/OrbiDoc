...
export const FilesWorkspace: React.FC<FilesWorkspaceProps> = ({
  ...
  const duplicate = (project: SavedProject) => {
    ...
    const clone: SavedProject = {
      ...
      createdAt: now,
      updatedAt: now,
      content: typeof structuredClone === 'function' ? structuredClone(project.content) : project.content,
    };
      ...
      <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <div className="p-5 sm:p-6 flex flex-col xl:flex-row xl:items-center gap-4">
          <div className="flex-1 min-w-0">
            <div className="inline-flex items-center gap-2 text-xs font-black text-indigo-600 dark:text-indigo-400"><Folder className="w-4 h-4" /> Biblioteca local</div>
            <h1 className="mt-1 text-2xl font-black text-slate-950 dark:text-white">Meus arquivos</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Projetos salvos neste dispositivo. A página Nuvem é separada para Google Drive e OneDrive.</p>
          </div>
          <div className="flex flex-wrap gap-2">{newActions.map((action) => { const Icon = action.icon; return <button key={action.type} onClick={() => onCreateProject(action.type)} className="h-10 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-[11px] font-black inline-flex items-center gap-2"><Icon className={`w-4 h-4 ${action.className}`} />{action.label}</button>; })}</div>
        </div>
      </section>
