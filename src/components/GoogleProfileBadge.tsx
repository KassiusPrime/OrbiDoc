...
export const GoogleProfileBadge: React.FC<GoogleProfileBadgeProps> = ({
  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      ...
      >
        {avatar ? (
          <img src={avatar} alt="" className="w-7 h-7 rounded-lg object-cover shrink-0" referrerPolicy="no-referrer" />
        ) : orbiUser ? (
          <div className="w-7 h-7 rounded-lg bg-[#3157F6] text-white flex items-center justify-center text-[9px] font-black shrink-0">{initialsFor(orbiUser)}</div>
        ) : (
        ...
        <div className="orbidoc-account-trigger-copy min-w-0 hidden sm:block text-left">
          ...
          <div className="text-[9px] text-slate-400 whitespace-nowrap">{orbiUser ? 'Conta OrbiDoc' : connectedCount ? `${connectedCount} serviço(s)` : 'Modo local'}</div>
        </div>
      ...
      {open && (
        <>
          <button
            type="button"
            className="fixed inset-0 z-[104] bg-slate-950/45 backdrop-blur-[1px] sm:hidden"
            onClick={() => setOpen(false)}
            aria-label="Fechar conta e conexões"
          />
          <div className="orbidoc-account-panel fixed left-3 right-3 top-[72px] bottom-[calc(86px+env(safe-area-inset-bottom))] z-[105] flex min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#101827] shadow-2xl sm:absolute sm:left-auto sm:right-0 sm:top-12 sm:bottom-auto sm:w-[390px] sm:max-w-[calc(100vw-24px)] sm:max-h-[calc(100dvh-80px)]">
...
const AccountRow: React.FC<{
  ...
  <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-3 flex items-center gap-3 hover:border-[#3157F6]/25 dark:hover:border-[#7AA2FF]/25">
    ...
    {connected ? <button onClick={onDisconnect} title={`Desconectar ${title}`} className="w-8 h-8 shrink-0 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/30 text-rose-500 flex items-center justify-center"><LogOut className="w-4 h-4" /></button> : <button onClick={onConnect} disabled={busy} className="h-8 px-2.5 shrink-0 rounded-lg bg-[#3157F6] hover:bg-[#2446D8] text-white text-[9px] font-black disabled:opacity-50">{busy ? 'Abrindo…' : 'Conectar'}</button>}
  </div>
);
