...
import { downloadDirectUrl, inferDirectDownloadKind, normalizeDirectDownloadUrl } from '../lib/linkDownloader';
import { OrbiDocLogo } from './OrbiDocLogo';

type Tab = 'enhance' | 'links';
type OutputType = 'image/png' | 'image/jpeg' | 'image/webp';
type EnhanceBusy = 'local' | 'ai' | null;
...
export const MediaToolsLauncher: React.FC = () => {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className="fixed z-[70] left-4 bottom-[84px] lg:bottom-5 h-12 lg:h-11 px-3 lg:px-4 rounded-2xl bg-white dark:bg-[#101827] border border-slate-200 dark:border-slate-700 shadow-xl text-slate-700 dark:text-slate-100 inline-flex items-center gap-2 text-[10px] font-black hover:border-[#3157F6]/40 active:scale-95 transition-transform" title="Mídia & qualidade · Ctrl+Shift+M" aria-label="Abrir ferramentas de mídia e qualidade"><Sparkles className="w-5 h-5 text-[#3157F6] dark:text-[#7AA2FF]" /><span className="hidden lg:inline">Mídia & qualidade</span></button>

    {open && <div className="fixed inset-0 z-[120] bg-[#F7F9FC] dark:bg-[#080D18] flex flex-col">
      <header className="h-14 shrink-0 px-3 sm:px-5 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-[#101827]/95 backdrop-blur flex items-center gap-3"><OrbiDocLogo size="sm" /><div className="h-5 w-px bg-slate-200 dark:bg-slate-700" /><div className="min-w-0 flex-1"><div className="text-xs font-black">Mídia & qualidade</div><div className="text-[9px] text-slate-400">Aprimoramento HQ, super-resolução e downloads diretos</div></div><button onClick={() => setOpen(false)} className="w-9 h-9 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center" aria-label="Fechar"><X className="w-4 h-4" /></button></header>
      <nav className="px-3 sm:px-5 pt-3 flex gap-2 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101827]"><TabButton active={tab === 'enhance'} onClick={() => setTab('enhance')} icon={<ImageIcon className="w-4 h-4" />} label="Aprimorar imagem" /><TabButton active={tab === 'links'} onClick={() => setTab('links')} icon={<Link className="w-4 h-4" />} label="Baixar por link" /></nav>
      <main className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-5">{tab === 'enhance' ? <ImageEnhancementStudio /> : <DirectLinkDownloader />}</main>
    </div>}
  </>;
};

const TabButton: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => <button type="button" onClick={onClick} className={`h-10 px-3 rounded-t-xl text-[10px] font-black inline-flex items-center gap-2 border-b-2 ${active ? 'border-[#3157F6] text-[#3157F6] dark:text-[#7AA2FF]' : 'border-transparent text-slate-500'}`}>{icon}{label}</button>;
...
const ImageEnhancementStudio: React.FC = () => {
  ...
  const [busy, setBusy] = useState<EnhanceBusy>(null);
  ...
  const commitResult = (blob: Blob, meta: string) => {
    if (resultUrl.startsWith('blob:')) URL.revokeObjectURL(resultUrl);
  ...
  const runLocal = async () => {
    ...
    try {
      ...
      commitResult(result.blob, `HQ local · ${result.width}×${result.height} · escala efetiva ${effective}×${result.plan.capped ? ' · limitada para proteger a memória' : ''}`);
      if (result.plan.capped) setNotice('A escala foi limitada automaticamente para impedir estouro de memória neste dispositivo.');
    } catch (error) {
  ...
  const runAi = async () => {
    if (!file) return;
    if (scale === 1) { setNotice('Super-resolução IA exige escala 2× ou 4×.'); return; }
    setBusy('ai');
    setNotice('');
    try {
      const image = await readAsDataUrl(file);
      const serverProfile = profile === 'anime' || profile === 'document' ? profile : 'photo';
      const response = await fetch('/api/enhance-image', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ image, scale, profile: serverProfile }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || `Falha no restaurador (${response.status}).`);
      if (!String(data.imageUrl || '').startsWith('data:image/')) throw new Error('O restaurador não retornou uma imagem válida.');
      const blob = await dataUrlToBlob(String(data.imageUrl));
      commitResult(blob, `${data.provider || 'IA'} · ${data.model || 'restauração'} · ${scale}× solicitado`);
      if (data.warning) setNotice(String(data.warning));
    } catch (error) {
      setNotice(error instanceof Error ? error.message : 'Falha na restauração por IA.');
    } finally { setBusy(null); }
  };
  ...
  return <div className="max-w-[1500px] mx-auto grid xl:grid-cols-[360px_minmax(0,1fr)] gap-4">
    <aside className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-5 shadow-sm space-y-4">
      ...
      <button disabled={!file || busy !== null || scale === 1} onClick={() => void runAi()} className="w-full h-10 rounded-xl border border-[#6D5EF7]/40 text-[#5B4FE0] dark:text-[#9D94FF] text-[10px] font-black inline-flex items-center justify-center gap-2 disabled:opacity-40"><Sparkles className="w-4 h-4" /> {busy === 'ai' ? 'Restaurando…' : 'Super-resolução IA'}</button>
      <div className="text-[8px] leading-relaxed text-slate-400">Para documentos oficiais ou imagens em que fidelidade absoluta é necessária, prefira HQ local. IA pode reconstruir microdetalhes.</div>
...
const DirectLinkDownloader: React.FC = () => {
  ...
  const normalized = useMemo(() => {
    try { return normalizeDirectDownloadUrl(url); } catch { return null; }
  }, [url]);
  const kind = useMemo(() => normalized ? inferDirectDownloadKind(normalized.toString()) : 'file', [normalized]);

  const download = async () => {
    if (!normalized) { setNotice('Use uma URL HTTP/HTTPS direta e válida.'); return; }
    setBusy(true);
    setProgress(null);
    setNotice('');
    try {
      const result = await downloadDirectUrl(normalized.toString(), (next) => setProgress(next));
      const finalName = safeFileName(name.trim() || result.filename, 'download');
      saveAs(result.blob, finalName);
      setNotice(`Download concluído: ${finalName} · ${(result.size / 1024 / 1024).toFixed(1)} MB · ${result.kind}`);
    } catch (error) {
  ...
  return <div className="max-w-4xl mx-auto space-y-4">
    <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 sm:p-6 shadow-sm"><div className="text-xs font-black text-[#3157F6] dark:text-[#7AA2FF] inline-flex items-center gap-2"><Link className="w-4 h-4" /> Download direto por URL</div><h1 className="mt-1 text-2xl font-black">Imagens, vídeos, músicas e arquivos por link direto</h1><p className="mt-2 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">O arquivo vai direto da origem para seu navegador. O OrbiDoc não funciona como proxy e não extrai mídia de páginas, DRM, login, paywall ou serviços de streaming.</p></section>
    <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm space-y-4">
      <label className="block"><span className="text-[9px] font-black uppercase tracking-wider text-slate-400">URL direta</span><input value={url} onChange={(event) => { setUrl(event.target.value); setNotice(''); setProgress(null); }} placeholder="https://exemplo.com/video.mp4" inputMode="url" className="mt-2 w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm outline-none focus:border-[#3157F6]" /></label>
      ...
      {normalized && <div className="grid sm:grid-cols-2 gap-2 text-[9px]"><div className="rounded-xl border border-slate-200 dark:border-slate-800 p-3"><div className="text-slate-400">Tipo inferido</div><div className="mt-1 font-black uppercase">{kind}</div></div><div className="rounded-xl border border-slate-200 dark:border-slate-800 p-3"><div className="text-slate-400">Origem</div><div className="mt-1 font-black truncate">{normalized.hostname}</div></div></div>}
      {progress !== null && <div><div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden"><div className="h-full bg-[#3157F6] transition-[width]" style={{ width: `${progress}%` }} /></div><div className="mt-1 text-[8px] text-slate-400 text-right">{progress}%</div></div>}
      ...
      <div className="flex flex-wrap gap-2"><button disabled={!normalized || busy} onClick={() => void download()} className="h-10 px-4 rounded-xl bg-[#3157F6] hover:bg-[#2446D8] text-white text-[10px] font-black inline-flex items-center gap-2 disabled:opacity-40"><Download className="w-4 h-4" /> {busy ? 'Baixando…' : 'Baixar arquivo'}</button><button disabled={!normalized} onClick={() => normalized && window.open(normalized.toString(), '_blank', 'noopener,noreferrer')} className="h-10 px-4 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-black inline-flex items-center gap-2 disabled:opacity-40"><ExternalLink className="w-4 h-4" /> Abrir origem</button><button disabled={!normalized} onClick={() => normalized && navigator.clipboard?.writeText(normalized.toString()).then(() => setNotice('Link copiado.')).catch(() => setNotice('Não foi possível copiar o link.'))} className="h-10 px-4 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-black inline-flex items-center gap-2 disabled:opacity-40"><Copy className="w-4 h-4" /> Copiar</button></div>
      <div className="rounded-xl border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/25 p-3 text-[9px] leading-relaxed text-amber-800 dark:text-amber-200"><strong>Uso permitido:</strong> conteúdo próprio, licenciado, domínio público ou cujo servidor ofereça download direto. URLs HLS/DASH (`.m3u8`/`.mpd`) são recusadas; se a origem bloquear CORS, use “Abrir origem” ou uma fonte que permita download direto.</div>
    </section>
  </div>;
};
