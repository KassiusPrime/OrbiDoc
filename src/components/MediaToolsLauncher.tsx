...
const DirectLinkDownloader: React.FC = () => {
  ...
  return <div className="max-w-4xl mx-auto space-y-4">
    <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 sm:p-6 shadow-sm"><div className="text-xs font-black text-[#3157F6] dark:text-[#7AA2FF] inline-flex items-center gap-2"><Link className="w-4 h-4" /> Download direto por URL</div><h1 className="mt-1 text-2xl font-black">Imagens, vídeos, músicas e arquivos por link direto</h1><p className="mt-2 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">O arquivo vai direto da origem para seu navegador. O OrbiDoc não funciona como proxy, não extrai mídia de páginas e não contorna DRM, login, paywall ou streaming.</p></section>
    <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm space-y-4">
      ...
      <div className="rounded-xl border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/25 p-3 text-[9px] leading-relaxed text-amber-800 dark:text-amber-200"><strong>Uso permitido:</strong> conteúdo próprio, licenciado, domínio público ou cujo servidor ofereça download. URLs HLS/DASH (`.m3u8`/`.mpd`) são recusadas e bloqueios CORS não são contornados.</div>
    </section>
  </div>;
};
