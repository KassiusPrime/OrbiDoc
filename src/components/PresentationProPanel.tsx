...
import { IconChecklist as Checklist, IconLayout as Layout, IconList as List, IconNumbers as Numbers } from '@tabler/icons-react';
import type { SavedProject } from '../types';
import { applyMasterFooter, auditDeck, createAgendaSlide, removeMasterFooter } from '../lib/presentationPro';
...
export const PresentationProPanel: React.FC<Props> = ({ project, onProjectChange, onApplied, showNotification = () => {} }) => {
  ...
  return <section className="mb-3 rounded-2xl border border-blue-200/70 dark:border-blue-900 bg-blue-50/50 dark:bg-blue-950/10 overflow-hidden">
    <button onClick={() => setOpen((value) => !value)} className="w-full min-h-11 px-3 sm:px-4 flex items-center gap-2 text-left"><Layout className="w-4 h-4 text-blue-600" /><span className="text-xs font-black">Apresentação Pro</span><span className="text-[9px] text-slate-500 dark:text-slate-400">Mestre · agenda · auditoria</span><span className="ml-auto text-[10px] font-black text-blue-700 dark:text-blue-300">{open ? 'Recolher' : 'Abrir'}</span></button>
    {open && <div className="p-3 sm:p-4 border-t border-blue-200/60 dark:border-blue-900 grid grid-cols-1 xl:grid-cols-3 gap-3">
