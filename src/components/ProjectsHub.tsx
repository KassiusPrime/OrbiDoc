...
type ProjectType = SavedProject['type'];
type FilterType = 'all' | ProjectType;
type SortType = 'updated' | 'name' | 'type';

const STORAGE_KEY = 'docswiss_projects_v1';
const FAVORITES_KEY = 'docswiss_project_favorites';
...
const DEFAULT_PROJECTS: SavedProject[] = [
  {
    ...
    previewSnippet: 'Proposta de prestação de serviços com cronograma, orçamento e análise de riscos.',
...
const typeMeta = {
  ...
  excel: {
    ...
    createClass: 'bg-emerald-600',
    tags: ['Planilha', 'XLSX'],
    newLabel: 'Nova Planilha',
  },
  powerpoint: {
    label: 'Apresentação',
    icon: Presentation,
    iconClass: 'bg-orange-50 text-orange-600 dark:bg-orange-950/60 dark:text-orange-300',
    createClass: 'bg-orange-600',
    tags: ['Apresentação', 'PPTX'],
    newLabel: 'Nova Apresentação',
  },
  canva: {
    label: 'Design',
    icon: PenTool,
    iconClass: 'bg-fuchsia-50 text-fuchsia-600 dark:bg-fuchsia-950/60 dark:text-fuchsia-300',
    createClass: 'bg-fuchsia-600',
    tags: ['Design'],
...
export const ProjectsHub: React.FC<ProjectsHubProps> = ({
  ...
  const createProject = (type: ProjectType) => {
    ...
    const project: SavedProject = {
      ...
      title: `${meta.newLabel} ${new Date().toLocaleDateString('pt-BR')}`,
      type,
      createdAt: now,
      updatedAt: now,
    ...
    // Persist immediately so the editor opens with the same stable project ID
    // that appears in Meus Arquivos. This prevents the shell autosave from
    // treating a freshly-created document as an anonymous, ever-changing item.
    const nextProjects = [project, ...projects];
    setProjects(nextProjects);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(nextProjects));
    onOpenProject(project);
    showNotification(`${project.title} criado e aberto na área de trabalho.`);
  };

  const filteredProjects = useMemo(() => {
    const query = searchQuery.trim().toLowerCase();
    const result = projects.filter((project) => {
      const typeMatches = selectedFilter === 'all' || project.type === selectedFilter;
  ...
  }, [projects, favorites, searchQuery, selectedFilter, sortBy]);
  ...
  const createTypes: ProjectType[] = ['word', 'excel', 'powerpoint', 'canva', 'extract'];
  const filterOptions: Array<{ value: FilterType; label: string }> = [
    { value: 'all', label: 'Todos' },
    { value: 'word', label: 'Word' },
    { value: 'excel', label: 'Excel' },
    { value: 'powerpoint', label: 'Slides' },
    { value: 'canva', label: 'Design' },
    ...
    <div className="space-y-5 animate-[fadeIn_0.2s_ease]">
      <header className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <div className="px-5 sm:px-6 py-5 flex flex-col lg:flex-row lg:items-center gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Documentos, planilhas, apresentações, designs e digitalizações em um único workspace.</p>
          </div>

          <div className="flex flex-wrap gap-2">
            <button onClick={() => setIsBatchExportOpen(true)} className="h-10 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold flex items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-700">
              <Download className="w-4 h-4" /> Exportar
            </button>
            <button onClick={() => createProject('word')} className="h-10 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold flex items-center gap-2 shadow-sm">
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 px-5 sm:px-6 py-3 bg-slate-50/60 dark:bg-slate-950/30 flex flex-wrap items-center gap-4 text-xs text-slate-600 dark:text-slate-300">
          ...
          <span className="inline-flex items-center gap-1.5"><Clock className="w-4 h-4 text-slate-400" /> IDs estáveis por projeto</span>
        </div>
      </header>
      ...
      <section>
        <div className="mb-3 px-1">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Escolha o formato e entre direto no editor.</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {createTypes.map((type) => {
            ...
            return (
              <button key={type} onClick={() => createProject(type)} className="group rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-3.5 text-left hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-md transition-all">
                <div className={`w-9 h-9 ${meta.createClass} text-white rounded-xl flex items-center justify-center shadow-sm`}><Icon className="w-4.5 h-4.5" /></div>
                <div className="mt-3 flex items-center justify-between gap-2">
                  <span className="text-xs font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{meta.label}</span>
      ...
      <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col xl:flex-row xl:items-center gap-3">
          <div className="relative flex-1 min-w-0">
            ...
            <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Pesquisar arquivos, tags ou conteúdo..." className="w-full h-10 pl-10 pr-9 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
            {searchQuery && <button onClick={() => setSearchQuery('')} aria-label="Limpar pesquisa" className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"><X className="w-4 h-4" /></button>}
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <div className="flex gap-1 rounded-xl bg-slate-100 dark:bg-slate-800 p-1 overflow-x-auto">
              {filterOptions.map((option) => (
                <button key={option.value} onClick={() => setSelectedFilter(option.value)} className={`h-8 px-3 rounded-lg text-[11px] font-bold whitespace-nowrap ${selectedFilter === option.value ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-300 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>{option.label}</button>
              ))}
            </div>
            ...
            <label className="h-10 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300">
              ...
              <select value={sortBy} onChange={(event) => setSortBy(event.target.value as SortType)} className="bg-transparent outline-none cursor-pointer">
            ...
            <div className="flex rounded-xl border border-slate-200 dark:border-slate-700 p-1">
              <button onClick={() => setViewMode('grid')} aria-label="Visualizar em grade" className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-slate-100 dark:bg-slate-800 text-indigo-600' : 'text-slate-400'}`}><Grid className="w-4 h-4" /></button>
              <button onClick={() => setViewMode('list')} aria-label="Visualizar em lista" className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-800 text-indigo-600' : 'text-slate-400'}`}><List className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        ...
        {filteredProjects.length === 0 ? (
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {filteredProjects.map((project) => {
              ...
              return (
                <article key={project.id} onClick={() => onOpenProject(project)} className="group rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 hover:bg-white dark:hover:bg-slate-800/60 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-md transition-all cursor-pointer overflow-hidden">
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className={`w-11 h-11 rounded-xl ${meta.iconClass} flex items-center justify-center`}><Icon className="w-5 h-5" /></div>
                      <button onClick={(event) => toggleFavorite(event, project.id)} className={`p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 ${favorite ? 'text-amber-500' : 'text-slate-300 dark:text-slate-600'}`} title="Favoritar"><Star className="w-4 h-4" fill={favorite ? 'currentColor' : 'none'} /></button>
                    </div>

                    {editingTitleId === project.id ? (
                      <div className="mt-4" onClick={(event) => event.stopPropagation()}>
                        <input autoFocus value={editTitleValue} onChange={(event) => setEditTitleValue(event.target.value)} onKeyDown={(event) => {
                          if (event.key === 'Enter') saveRename(project.id);
                          if (event.key === 'Escape') setEditingTitleId(null);
                        }} onBlur={() => saveRename(project.id)} className="w-full h-9 px-2.5 rounded-lg border border-indigo-400 bg-white dark:bg-slate-900 text-sm font-bold outline-none" />
                      </div>
                    ) : (
      ...
      <BatchExportModal isOpen={isBatchExportOpen} onClose={() => setIsBatchExportOpen(false)} projects={projects} showNotification={showNotification} />
    </div>
  );
};
