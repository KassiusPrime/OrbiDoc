      ...
      <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <div className="p-5 sm:p-6 flex flex-col xl:flex-row xl:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-3xl">Câmera e scanner multipágina, leitor local de PDF/EPUB/ZIP/HTML/TXT/Markdown/DOCX/planilhas e OCR local.</p>
          </div>
...
const ScannerStudio: React.FC<Props & { notify: (message: string, type?: 'success' | 'error') => void; onOpenOcr: () => void }> = ({ items, setItems, notify, onSaveToHistory, onOpenOcr }) => {
  ...
  useEffect(() => () => pages.forEach(releaseScanPage), []);
  ...
  }, [active?.id, active?.rotation, active?.filter, active?.brightness, active?.contrast, active?.crop.left, active?.crop.top, active?.crop.right, active?.crop.bottom]);
  ...
  const autoCrop = async () => {
    if (!active) return;
    setBusy('crop');
    try { patchActive({ crop: await autoDetectScanCrop(active) }); notify('Recorte automático estimado. Revise as bordas antes de exportar.', 'success'); }
    catch { notify('Não foi possível detectar as bordas automaticamente.', 'error'); }
  ...
  return (
    <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
      ...
      <div className="grid lg:grid-cols-[180px_minmax(0,1fr)_300px] min-h-[650px]">
        <aside className="border-r border-slate-100 dark:border-slate-800 p-2 overflow-y-auto max-h-[780px]">
          ...
          <div className="space-y-2">
            {pages.map((page, index) => (
              <button key={page.id} onClick={() => setActiveId(page.id)} className={`w-full rounded-xl border p-2 text-left ${active?.id === page.id ? 'border-[#3157F6] bg-[#EFF4FF] dark:bg-[#0D1E5B]/40' : 'border-slate-200 dark:border-slate-800'}`}>
                ...
                <div className="mt-1.5 text-[9px] font-black">Página {index + 1}</div>
              </button>
            ))}
          </div>
        </aside>
        ...
        <aside className="border-l border-slate-100 dark:border-slate-800 p-3 overflow-y-auto max-h-[780px]">
          {active ? <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2"><button onClick={() => patchActive({ rotation: ((active.rotation + 90) % 360) as ScanPage['rotation'] })} className="h-9 rounded-xl border text-[9px] font-black inline-flex items-center justify-center gap-1.5"><Rotate className="w-3.5 h-3.5" /> Girar 90°</button><button disabled={busy === 'crop'} onClick={() => void autoCrop()} className="h-9 rounded-xl border text-[9px] font-black inline-flex items-center justify-center gap-1.5"><Crop className="w-3.5 h-3.5" /> Auto recorte</button></div>
...
const ReaderStudio: React.FC<{ notify: (message: string, type?: 'success' | 'error') => void; onSendToAiText?: (text: string) => void }> = ({ notify, onSendToAiText }) => {
  ...
  useEffect(() => () => releaseReaderDocument(document), []);
  ...
  return <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
    ...
    <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-2">
      ...
      <select value={readerTheme} onChange={(event) => setReaderTheme(event.target.value as any)} className="h-9 rounded-xl border bg-white dark:bg-slate-950 px-2 text-[10px] font-bold"><option value="paper">Papel</option><option value="sepia">Sépia</option><option value="dark">Escuro</option></select>
