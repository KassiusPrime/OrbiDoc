...
export function getSupportedOutputs(file: File): ConvertibleFormat[] {
  ...
  if (IMAGE_FORMATS.has(extension)) {
    return (['png', 'jpg', 'webp', 'avif', 'pdf', 'txt', 'html', 'docx'] as ConvertibleFormat[]).filter((format) => format !== extension);
  }
...
function textToHtml(text: string, title: string) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (/^[-*•]\s+/.test(trimmed)) {
      ...
      parts.push(`<li>${escapeHtml(trimmed.replace(/^[-*•]\s+/, ''))}</li>`); continue;
    }
    if (/^\d+[.)]\s+/.test(trimmed)) {
      if (list !== 'ol') { closeList(); list = 'ol'; parts.push('<ol>'); }
      parts.push(`<li>${escapeHtml(trimmed.replace(/^\d+[.)]\s+/, ''))}</li>`); continue;
    }
    if (/^>\s+/.test(trimmed)) { closeList(); parts.push(`<blockquote>${escapeHtml(trimmed.replace(/^>\s+/, ''))}</blockquote>`); continue; }
    closeList(); parts.push(`<p>${escapeHtml(rawLine)}</p>`);
  }
  closeList();
...
type StructuredBlock = { kind: 'h1' | 'h2' | 'h3' | 'paragraph' | 'bullet' | 'number' | 'quote' | 'code'; text: string };

function textToBlocks(text: string): StructuredBlock[] {
  let inCode = false;
  let codeBuffer: string[] = [];
  const blocks: StructuredBlock[] = [];
  const flushCode = () => { if (codeBuffer.length) blocks.push({ kind: 'code', text: codeBuffer.join('\n') }); codeBuffer = []; };
  for (const rawLine of text.split(/\r?\n/)) {
    const trimmed = rawLine.trim();
    if (trimmed.startsWith('```')) { if (inCode) flushCode(); inCode = !inCode; continue; }
...
async function blocksToPdf(blocks: StructuredBlock[], title: string) {
  ...
  let y = 20;
  const addPageIfNeeded = (needed: number) => { if (y + needed <= height - 18) return; pdf.addPage(); y = 20; };
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(18); pdf.setTextColor(15, 23, 42);
  const titleLines = pdf.splitTextToSize(title, contentWidth); pdf.text(titleLines, margin, y); y += titleLines.length * 7 + 5;
  ...
  for (const block of blocks) {
    ...
    pdf.setFont(block.kind === 'code' ? 'courier' : 'helvetica', style); pdf.setFontSize(fontSize);
    ...
    const blockHeight = Math.max(1, lines.length) * spacing + (block.kind.startsWith('h') ? 3 : 2); addPageIfNeeded(blockHeight);
    if (block.kind === 'quote') { pdf.setDrawColor(99, 102, 241); pdf.setLineWidth(0.8); pdf.line(margin, y - 3, margin, y + Math.max(spacing, lines.length * spacing - 2)); }
    if (block.kind === 'code') { pdf.setFillColor(248, 250, 252); pdf.roundedRect(margin, y - 4, contentWidth, blockHeight, 2, 2, 'F'); }
    pdf.text(lines, margin + indent, y); y += blockHeight;
  }
...
async function blocksToDocx(blocks: StructuredBlock[], title: string) {
  ...
  for (const block of blocks) {
    ...
    if (block.kind === 'quote') { children.push(new docx.Paragraph({ children: [new docx.TextRun({ text: block.text, italics: true, color: '475569' })], indent: { left: 420 } })); continue; }
    if (block.kind === 'code') { children.push(new docx.Paragraph({ children: [new docx.TextRun({ text: block.text, font: 'Courier New', size: 18 })], shading: { fill: 'F8FAFC' } })); continue; }
    children.push(new docx.Paragraph({ text: block.text, spacing: { after: 120 } }));
  }
  const document = new docx.Document({ numbering: { config: [{ reference: numberingReference, levels: [{ level: 0, format: docx.LevelFormat.DECIMAL, text: '%1.', alignment: docx.AlignmentType.START, style: { paragraph: { indent: { left: 720, hanging: 260 } } } }] }] }, sections: [{ children }] });
  return docx.Packer.toBlob(document);
}

async function loadImage(file: File) {
  if (typeof createImageBitmap === 'function') { try { return await createImageBitmap(file); } catch { /* fall through */ } }
...
async function imageToCanvas(file: File, options: ConversionOptions) {
  ...
  const width = Math.max(1, Math.round(sourceWidth * ratio)); const height = Math.max(1, Math.round(sourceHeight * ratio));
  const canvas = document.createElement('canvas'); canvas.width = width; canvas.height = height;
  const context = canvas.getContext('2d'); if (!context) throw new Error('Canvas 2D indisponível neste navegador.');
  context.imageSmoothingEnabled = true; context.imageSmoothingQuality = 'high'; context.drawImage(image as CanvasImageSource, 0, 0, width, height);
...
async function canvasToBlob(canvas: HTMLCanvasElement, format: 'png' | 'jpg' | 'webp' | 'avif', quality = 0.9) {
  ...
  if (format === 'jpg') {
    renderCanvas = document.createElement('canvas'); renderCanvas.width = canvas.width; renderCanvas.height = canvas.height;
    const context = renderCanvas.getContext('2d'); if (!context) throw new Error('Canvas 2D indisponível neste navegador.');
    context.fillStyle = '#ffffff'; context.fillRect(0, 0, canvas.width, canvas.height); context.drawImage(canvas, 0, 0);
  }
  const mime = MIME_BY_FORMAT[format];
  const blob = await new Promise<Blob>((resolve, reject) => renderCanvas.toBlob((value) => value ? resolve(value) : reject(new Error(`Falha ao codificar ${format.toUpperCase()}.`)), mime, quality));
...
async function imageToPdf(file: File, options: ConversionOptions) {
  const canvas = await imageToCanvas(file, options); const { jsPDF } = await import('jspdf');
  const landscape = canvas.width > canvas.height; const pdf = new jsPDF({ orientation: landscape ? 'landscape' : 'portrait', unit: 'mm', format: 'a4' });
...
async function readPdf(file: File) {
  const pdfjs = await import('pdfjs-dist'); const workerModule = await import('pdfjs-dist/build/pdf.worker.min.mjs?url');
...
async function pdfToImageArchive(file: File, target: 'png' | 'jpg' | 'webp' | 'avif', options: ConversionOptions): Promise<ConversionResult> {
  const pdf = await readPdf(file); const quality = Math.min(1, Math.max(0.1, options.quality ?? 0.9)); const scale = Math.min(3, Math.max(1, options.pdfScale ?? 1.75));
  const pageBlobs: Blob[] = [];
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    const page = await pdf.getPage(pageNumber); const viewport = page.getViewport({ scale }); const canvas = document.createElement('canvas');
    canvas.width = Math.ceil(viewport.width); canvas.height = Math.ceil(viewport.height); const context = canvas.getContext('2d'); if (!context) throw new Error('Canvas 2D indisponível.');
    await page.render({ canvasContext: context, viewport }).promise; pageBlobs.push(await canvasToBlob(canvas, target, quality));
  }
  const base = cleanBaseName(file.name);
  if (pageBlobs.length === 1) return { blob: pageBlobs[0], fileName: `${base}.${target}`, mimeType: MIME_BY_FORMAT[target], warnings: [] };
  const JSZipModule = await import('jszip'); const JSZip = (JSZipModule as any).default || JSZipModule; const zip = new JSZip();
...
async function docxToHtml(file: File) {
  const module = await import('mammoth'); const mammoth = (module as any).default || module;
...
async function spreadsheetToData(file: File) {
  const module = await import('xlsx'); const XLSX = (module as any).default || module; const workbook = XLSX.read(await file.arrayBuffer(), { type: 'array' });
  const sheetName = workbook.SheetNames[0]; if (!sheetName) throw new Error('A planilha não contém abas legíveis.'); const sheet = workbook.Sheets[sheetName];
  return { XLSX, workbook, sheet, sheetName, csv: XLSX.utils.sheet_to_csv(sheet), html: XLSX.utils.sheet_to_html(sheet), rows: XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false }) as any[][] };
}

async function csvToXlsx(file: File) {
  const module = await import('xlsx'); const XLSX = (module as any).default || module; const workbook = XLSX.read(await file.text(), { type: 'string' });
  const array = XLSX.write(workbook, { type: 'array', bookType: 'xlsx' }); return new Blob([array], { type: MIME_BY_FORMAT.xlsx });
}
...
async function textResult(text: string, base: string, target: 'txt' | 'html' | 'docx' | 'pdf', warnings: string[]): Promise<ConversionResult> {
  ...
  if (target === 'html') { const html = textToHtml(text, base); return { blob: new Blob([html], { type: MIME_BY_FORMAT.html }), fileName: `${base}.html`, mimeType: MIME_BY_FORMAT.html, warnings }; }
...
export async function convertFile(file: File, target: ConvertibleFormat, options: ConversionOptions = {}): Promise<ConversionResult> {
  const extension = getFileExtension(file); const supported = getSupportedOutputs(file);
  ...
  const base = cleanBaseName(file.name); const warnings: string[] = []; const quality = Math.min(1, Math.max(0.1, options.quality ?? 0.9));
  ...
  if (IMAGE_FORMATS.has(extension)) {
    ...
    if (target === 'png' || target === 'jpg' || target === 'webp' || target === 'avif') {
      const canvas = await imageToCanvas(file, options); const blob = await canvasToBlob(canvas, target, quality);
    ...
    const text = await runOcr(file, options); if (!text) warnings.push('O OCR não encontrou texto legível na imagem.');
  ...
  if (extension === 'pdf') {
    ...
    const text = await runOcr(file, options); if (!text) warnings.push('Nenhum texto legível foi encontrado no PDF.');
    return textResult(text, base, target as 'txt' | 'html' | 'docx', warnings);
  }

  if (extension === 'docx') {
    const converted = await docxToHtml(file); warnings.push(...converted.warnings);
    if (target === 'html') { const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${escapeHtml(base)}</title></head><body>${converted.html}</body></html>`; return { blob: new Blob([html], { type: MIME_BY_FORMAT.html }), fileName: `${base}.html`, mimeType: MIME_BY_FORMAT.html, warnings }; }
  ...
  if (extension === 'xlsx' || extension === 'xls') {
    ...
    if (target === 'html') { const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${escapeHtml(base)}</title><style>body{font-family:Arial,sans-serif;padding:24px}table{border-collapse:collapse;width:100%}td,th{border:1px solid #cbd5e1;padding:6px 8px;text-align:left}</style></head><body><h1>${escapeHtml(base)}</h1>${data.html}</body></html>`; return { blob: new Blob([html], { type: MIME_BY_FORMAT.html }), fileName: `${base}.html`, mimeType: MIME_BY_FORMAT.html, warnings }; }
  ...
  if (extension === 'csv') {
    ...
    if (target === 'html') { const rows = text.split(/\r?\n/).filter(Boolean).map((line) => line.split(',').map((cell) => `<td>${escapeHtml(cell)}</td>`).join('')); const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${escapeHtml(base)}</title></head><body><table>${rows.map((row) => `<tr>${row}</tr>`).join('')}</table></body></html>`; return { blob: new Blob([html], { type: MIME_BY_FORMAT.html }), fileName: `${base}.html`, mimeType: MIME_BY_FORMAT.html, warnings }; }
    if (target === 'txt') return { blob: new Blob([text], { type: MIME_BY_FORMAT.txt }), fileName: `${base}.txt`, mimeType: MIME_BY_FORMAT.txt, warnings };
    if (target === 'docx') return { blob: await blocksToDocx([{ kind: 'code', text }], base), fileName: `${base}.docx`, mimeType: MIME_BY_FORMAT.docx, warnings };
    return { blob: await blocksToPdf([{ kind: 'code', text }], base), fileName: `${base}.pdf`, mimeType: MIME_BY_FORMAT.pdf, warnings };
  }
  ...
  if (TEXT_FORMATS.has(extension)) {
    const text = await file.text(); return textResult(text, base, target as 'html' | 'pdf' | 'docx', warnings);
  }
