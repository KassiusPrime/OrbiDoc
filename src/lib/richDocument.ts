...
export function sanitizeRichHtml(source: string): string {
  ...
  for (const element of elements) {
    ...
    for (const attribute of Array.from(element.attributes)) {
      ...
      if (name === 'style') {
        const safeDeclarations = attribute.value.split(';').map((item) => item.trim()).filter(Boolean).filter((item) => /^(text-align|color|background-color|font-weight|font-style|text-decoration|font-family|font-size|max-width|width|height)\s*:/i.test(item) && !/url\s*\(|expression\s*\(|javascript:/i.test(item));
...
function parseCssColor(value: string | null): string | undefined {
...
export async function richHtmlToDocxBlob(html: string, title: string): Promise<Blob> {
  ...
  const textRuns = (node: Node, inherited: { bold?: boolean; italics?: boolean; underline?: boolean; strike?: boolean; color?: string; font?: string; size?: number } = {}): any[] => {
    ...
    if (node.tagName === 'U' || /text-decoration[^;]*underline/i.test(style)) next.underline = true;
    ...
    if (node.tagName === 'A' && node.getAttribute('href')) {
      const children = Array.from(node.childNodes).flatMap((child) => textRuns(child, { ...next, color: next.color || '2563EB', underline: true }));
  ...
  const paragraphFrom = (element: Element, options: any = {}) => {
    const style = element.getAttribute('style') || '';
    const alignmentValue = style.match(/text-align\s*:\s*(left|center|right|justify)/i)?.[1]?.toLowerCase();
    const alignment = alignmentValue === 'center' ? docx.AlignmentType.CENTER : alignmentValue === 'right' ? docx.AlignmentType.RIGHT : alignmentValue === 'justify' ? docx.AlignmentType.JUSTIFIED : undefined;
  ...
  const walkBlock = (element: Element, listType?: 'bullet' | 'number', level = 0) => {
    if (/^H[1-6]$/.test(element.tagName)) {
      const heading = ({ H1: docx.HeadingLevel.HEADING_1, H2: docx.HeadingLevel.HEADING_2, H3: docx.HeadingLevel.HEADING_3, H4: docx.HeadingLevel.HEADING_4, H5: docx.HeadingLevel.HEADING_5, H6: docx.HeadingLevel.HEADING_6 } as any)[element.tagName];
    ...
    if (element.tagName === 'LI') {
      const opts = listType === 'number' ? { numbering: { reference: numberingReference, level: Math.min(level, 2) } } : { bullet: { level: Math.min(level, 2) } };
    ...
    if (element.tagName === 'TABLE') {
      const rows = Array.from(element.querySelectorAll(':scope > thead > tr, :scope > tbody > tr, :scope > tfoot > tr, :scope > tr')).map((row) => new docx.TableRow({ children: Array.from(row.children).filter((cell) => cell.tagName === 'TD' || cell.tagName === 'TH').map((cell) => new docx.TableCell({ children: [paragraphFrom(cell)], columnSpan: Math.max(1, Number(cell.getAttribute('colspan')) || 1), rowSpan: Math.max(1, Number(cell.getAttribute('rowspan')) || 1) })) }));
    ...
    if (element.tagName === 'FIGURE') {
      ...
      if (img?.getAttribute('src')?.startsWith('data:image/')) {
        try {
          ...
          children.push(new docx.Paragraph({ children: [new docx.ImageRun({ data: bytes, transformation: { width: 520, height: 320 }, type: src.includes('jpeg') || src.includes('jpg') ? 'jpg' : 'png' as any })], alignment: docx.AlignmentType.CENTER }));
        } catch { /* keep caption/text even if image cannot be encoded */ }
      }
  ...
  const doc = new docx.Document({
    ...
    creator: 'DocSwiss',
    numbering: {
      config: [{ reference: numberingReference, levels: [0, 1, 2].map((level) => ({ level, format: docx.LevelFormat.DECIMAL, text: `%${level + 1}.`, alignment: docx.AlignmentType.START, style: { paragraph: { indent: { left: 720 + level * 360, hanging: 260 } } } })) }],
    },
